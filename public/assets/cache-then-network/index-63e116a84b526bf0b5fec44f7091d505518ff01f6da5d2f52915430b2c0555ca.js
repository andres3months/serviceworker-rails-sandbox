...
var _companion = require('cache-then-network/companion');

var _init = require('cache-then-network/init');
...
},{"cache-then-network/companion":2,"cache-then-network/init":3}],2:[function(require,module,exports){
'use strict';
...
var logger = new _logger2.default('[cache-then-network/client]');
...
},{"utils/alertonce":5,"utils/logger":7}],3:[function(require,module,exports){
'use strict';
...
var _render = require('cache-then-network/render');
...
function fetchData() {
  ...
  }).catch(_render.renderError);
}

function deleteCache(e) {
  e.preventDefault();
  caches.delete((0, _caching.cacheKey)('cache-then-network')).then(function (success) {
    logger.log('Cached deleted', success);
    (0, _render.renderPage)({ force: true, message: "Cache is now deleted." }, state);
  });
}
...
},{"cache-then-network/render":4,"lodash":8,"utils/caching":6,"utils/logger":7}],4:[function(require,module,exports){
'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.renderError = exports.renderPage = undefined;
...
function renderError(e) {
  var msg = 'No data received from cache or network or error';
...
function renderDots(n) {
  ...
  if ($dots.length) {
    $dots.text(dots);
    setTimeout(function () {
      return renderDots(n + 1);
    }, 250);
  }
}

function tweetsToHtml(tweets) {
  return tweets.sort(function (tw, i) {
    return tw.id;
  }).map(tweetToHtml).join("");
}

function tweetToHtml(tweet) {
  var pics = (0, _lodash2.default)(tweet.entities.media).map(function (img) {
    return '<img src="' + img.media_url_https + '" class="tweet-img" />';
  }).join("\n");
